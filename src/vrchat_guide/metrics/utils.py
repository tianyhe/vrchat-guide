...
class MetricsManager:
    ...
    def __init__(self, metrics_dir: str, session_timestamp: str):
        self.logger = MetricsLogger(
            output_dir=metrics_dir, session_timestamp=session_timestamp
        )
        self.log_config = LogConfig()
        self.prompts_file = self.log_config.get_session_path(
            session_timestamp, "prompts"
        )
    ...
    def measure_latency(self, operation_name: str):
        ...
        try:
            ...
            self.latency_metrics[operation_name].append(duration)

            # Log to metrics logger
            self.logger.log_latency(operation_name, duration)
    ...
    def detect_task_type(self, dialogue_turn: CurrentDialogueTurn) -> Optional[str]:
        ...
        if any(
            word in user_utterance for word in ["profile", "preferences", "settings"]
        ):
    ...
    def handle_dialogue_turn(
        self, dialogue_turn: CurrentDialogueTurn, response_time: float
    ):
        ...
        if self._is_task_complete(dialogue_turn):
            self.logger.complete_task(
                success=True, notes=f"Completed task: {self.current_task}"
            )
